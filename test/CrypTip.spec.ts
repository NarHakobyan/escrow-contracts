  ...
  : describe('CrypTip Unit Tests', function () {
      ...
      describe('sendTip', function () {
        ...
        it('Updates contract tips record', async function () {
          const txResponse = await cryptip.sendTip(
            user.address,
            'Peter',
            'Gm',
            {
          ...
          const contractBalance = await cryptip.provider.getBalance(
            cryptip.address,
          );
      ...
      describe('withdrawTips', function () {
        it("Doesn't allow 0 tips withdrawals", async function () {
          await expect(cryptip.withdrawTips()).to.be.revertedWith(
            'CrypTip__NoTips',
          );
        });
        ...
        it('Withdraws tips', async function () {
          ...
          assert(
            addressBalance.add(gasCost).toString() == '10000100000000000000000',
          );
        });
      });
    });
