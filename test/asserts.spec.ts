...
import { assertRoughlyEqualValues } from './utils/asserts';
...
describe('assertRoughlyEqualValues', async function () {
  ...
  it('should be thrown with expected > actual more than relativeDiff * expected', async function () {
    expect(() =>
      assertRoughlyEqualValues(1000001, 1000000, 0.0000001),
    ).throws();
  });

  it('should be thrown with expected < actual more than relativeDiff * expected', async function () {
    expect(() =>
      assertRoughlyEqualValues(1000000, 1000001, 0.0000001),
    ).throws();
  });
});
